...
const ProjectCard = ({ title, category, imageUrl, isActive, onClick }) => {
  return (
    <div 
      className={`relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-500 ${isActive ? 'h-96' : 'h-64'}`}
      onClick={onClick}
    >
      ...
      />
      <div className={`absolute bottom-0 left-0 p-6 z-20 transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-80'}`}>
        <span className="text-xs font-semibold text-blue-300 uppercase tracking-wider">{category}</span>
        <h3 className="text-xl font-bold text-white mt-1">{title}</h3>
        {isActive && (
          <button className="mt-4 bg-white text-blue-900 text-sm font-bold py-2 px-4 rounded-full inline-flex items-center hover:bg-gray-100 transition-all">
            Ver proyecto
    ...
    <section className="py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <span className="text-sm font-semibold tracking-wider text-blue-600 uppercase">Nuestro Trabajo</span>
          <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mt-2 mb-4">Portafolio de Proyectos</h2>
        ...
        <div className="text-center">
          <button className="inline-flex items-center text-blue-600 font-bold group">
